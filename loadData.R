...
reut21578Labels <- data.frame(acq, corn, crude, earn, grain,
                              interest, money, ship, trade, wheat)

t <- read.table(file = "./data/reuters21578.10.training.matrix")

dataset <- sparseMatrix(t[,1], t[,2], x=t[, 3] <- 1)

#save(list = c("reut21578Labels", "dataset"), file = "./dataset.RData")
...
reut21578LabelsTest <- data.frame(acq, corn, crude, earn, grain,
                                  interest, money, ship, trade, wheat)
